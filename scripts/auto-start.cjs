...
const EnvironmentChecker = require('./env-checker.cjs');
const DependencyInstaller = require('./dep-installer.cjs');
const DevServerStarter = require('./dev-starter.cjs');
...
示例:
  node auto-start.cjs
  node auto-start.cjs --port 3000
  node auto-start.cjs --host 127.0.0.1 --no-open
  node auto-start.cjs --check-only
      `);
      process.exit(0);
    }
  }
